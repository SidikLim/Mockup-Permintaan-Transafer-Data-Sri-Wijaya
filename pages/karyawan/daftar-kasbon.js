/* ===========================================================
   ...
   daftar-kasbon.js - logika halaman Daftar Utang Kasbon Karyawan
   =========================================================== */
...
(function () {
  ...
  function renderTable() {
    ...
    if (!pageItems.length) {
      tbody.innerHTML = '<tr><td colspan="11" style="text-align:center;color:var(--text-muted);padding:24px;">Tidak ada data kas bon yang cocok.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = pageItems.map(function (t) {
      return (
        '<tr>' +
          '<td><a href="kasbon-form.html?no=' + encodeURIComponent(t.no) + '" style="color:var(--sidebar-active-text);font-weight:600;">' + MockUI.esc(t.no) + '</a></td>' +
          '<td>' + MockUI.esc(t.caNumber || '') + '</td>' +
          '<td>' + MockUI.esc(t.karyawan) + '</td>' +
          '<td>' + MockUI.esc(formatTgl(t.tglTrn)) + '</td>' +
          '<td>' + MockUI.esc(t.keterangan || '') + '</td>' +
          '<td class="num">' + MockUI.formatCurrency(t.jumlah) + '</td>' +
          '<td><a class="btn-icon blue" href="kasbon-form.html?no=' + encodeURIComponent(t.no) + '" title="Ubah">&#9998;</a></td>' +
    ...
    tbody.querySelectorAll('[data-view]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        MockUI.toast('info', 'Pratinjau kas bon berada di luar cakupan mockup ini. Gunakan tombol Ubah untuk melihat detail.');
      });
    });
    tbody.querySelectorAll('[data-print]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        MockUI.toast('info', 'Cetak bukti kas bon berada di luar cakupan mockup ini.');
      });
    });
  ...
  function onDelete(no) {
    var t = KasbonStore.getByNo(no);
    if (!t) return;
    MockUI.confirmDialog({
      title: 'Hapus Kas Bon',
      message: 'Hapus transaksi kas bon "' + t.no + '" (' + t.karyawan + ')?',
      confirmLabel: 'Hapus'
    }, function () {
      KasbonStore.removeByNo(no);
      state.all = KasbonStore.loadAll();
      applyFilter();
  ...
  function bindEvents() {
    ...
    document.getElementById('btnTambah').addEventListener('click', function () {
      window.location.href = 'kasbon-form.html';
    });
    document.getElementById('btnImport').addEventListener('click', function () {
      MockUI.toast('info', 'Import Transaksi Kas Bon berada di luar cakupan mockup tahap ini.');
    });
    ...
    document.getElementById('btnHelp').addEventListener('click', function () {
      MockUI.toast('info', 'Mockup Kas Bon Karyawan - lihat/cetak berada di luar cakupan mockup ini.', 4500);
    });
  }
